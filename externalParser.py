...
def interlock_check():
	...
	if GPIO.input(4) == 0:			## is interlock open?
		if GPIO.input(17) == 1:		## is override on?
			return '09'
		else:
			return '90'
	else:
...
def arg_check(test_args, known_args):
...
def laser_mains_CMD(args):
	...
	a_check = arg_check(args, [['ON', 'OFF']])
	i_check = interlock_check()	
	
	if a_check == '00':				## are arguments good?
	
		if args[0].upper() == 'ON':		## are you trying to turn it on?
			if laser("SOUR:AM:STAT?")[-1] == 'ON':	## already on?
				return '01'
			else:
				if i_check != '90':	## is interlock closed or being overridden?
					result = laser("SOUR:AM:STAT ON")[0]	## switch on laser
					if 	result == '00':	## laser switched on OK?
						return i_check
					else: return result
				else: return i_check
				
		if args[0].upper() == 'OFF': ## are you trying to turn it off?
			if laser("SOUR:AM:STAT?")[-1] == 'OFF':	## already off?
				return '01'
			else: return laser("SOUR:AM:STAT ON")[0]	## switch on laser
...
def laser_mode_CMD(args):
	a_check = arg_check(args, [['gated', 'master', 'indep']])
	return a_check
...
def laser_mod_polarity_CMD(args):
	a_check = arg_check(args, [['pass', 'invert']])
	return a_check
...
def laser_modulation_CMD(args):
	a_check = arg_check(args, [['sine', 'square', 'triangle', 'sawtooth', 'full'],
							   [0, 10000],
							   [0, 100]])
	return a_check
